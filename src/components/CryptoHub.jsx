...
export default function CryptoHub({ 
  ...
  return (
    <section id="crypto-hub" className="py-12 border-b border-white/[0.06]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-400 text-xs font-semibold mb-2.5 border border-indigo-500/20 font-mono uppercase tracking-wider">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5 mb-8">
          ...
          <div className="bg-[#0C1017] border border-white/[0.08] rounded-2xl p-4 shadow-sm">
            <div className="text-[11px] font-bold text-slate-400 uppercase font-mono mb-1.5 flex items-center justify-between">
            ...
            <div className="text-base sm:text-lg font-black text-white flex items-center gap-1.5">
              {stats.topGainer ? (
                <>
                  ...
                  <span className="text-xs font-mono text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20 font-bold">
          ...
          <div className="bg-[#0C1017] border border-white/[0.08] rounded-2xl p-4 shadow-sm">
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-xs font-bold font-mono text-emerald-400">64% Bullish</span>
              <span className="text-slate-600">/</span>
              <span className="text-xs font-bold font-mono text-rose-400">36% Bearish</span>
            </div>
            <div className="w-full bg-[#07090E] h-1.5 rounded-full overflow-hidden flex mt-2">
        ...
        {/* Table Container */}
        <div className="bg-[#0C1017] border border-white/[0.08] rounded-3xl p-5 sm:p-7 shadow-2xl">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-9 pr-3.5 py-1.5 bg-[#07090E] border border-white/[0.08] rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 w-full sm:w-56 font-mono"
                />
              </div>

              {/* Filter Tabs */}
              <div className="flex items-center gap-1 bg-[#07090E] p-1 rounded-xl border border-white/[0.08]">
                <button
                ...
                <button
                  ...
                  className={`text-xs px-3 py-1 rounded-lg font-semibold transition-all cursor-pointer ${
                    filterType === 'gainers' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  Gainers
                </button>
                <button
                  ...
                  className={`text-xs px-3 py-1 rounded-lg font-semibold transition-all cursor-pointer ${
                    filterType === 'losers' ? 'bg-rose-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  Losers
                </button>
          ...
          {/* Table */}
          {cryptoList.length > 0 && (
            <div className="overflow-x-auto mt-4 rounded-xl border border-white/[0.05]">
              <table className="w-full text-left border-collapse">
                <thead className="bg-[#07090E]">
                  <tr className="border-b border-white/[0.06] text-[10px] font-bold uppercase tracking-wider text-slate-400 font-mono">
                    <th onClick={() => handleSort('market_cap_rank')} className="py-3 px-3.5 cursor-pointer hover:text-white">
                    ...
                    <th onClick={() => handleSort('current_price')} className="py-3 px-3.5 text-right cursor-pointer hover:text-white">
                      <div className="flex items-center justify-end gap-1">
                        <span>Price (USD)</span>
                        <ArrowUpDown className="w-3 h-3" />
                      </div>
                    </th>
                    <th onClick={() => handleSort('price_change_percentage_24h')} className="py-3 px-3.5 text-right cursor-pointer hover:text-white">
                      <div className="flex items-center justify-end gap-1">
                    ...
                    <th onClick={() => handleSort('total_volume')} className="py-3 px-3.5 text-right hidden lg:table-cell cursor-pointer hover:text-white">
                      <div className="flex items-center justify-end gap-1">
                        <span>24h Volume</span>
                        <ArrowUpDown className="w-3 h-3" />
                      </div>
                    </th>
                    <th onClick={() => handleSort('market_cap')} className="py-3 px-3.5 text-right cursor-pointer hover:text-white">
                      <div className="flex items-center justify-end gap-1">
                  ...
                  {filteredCoins.map((coin) => {
                    ...
                    return (
                      <tr key={coin.id} className="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td className="py-3.5 px-3.5">
                          <div className="flex items-center gap-2.5">
                            {coin.image ? (
                              ...
                              <div className="w-6 h-6 rounded-full bg-blue-500/10 border border-blue-500/20 flex items-center justify-center font-bold text-[10px] text-blue-400">
                        ...
                        <td className="py-3.5 px-3.5 text-right font-mono tabular-nums font-bold text-white">
                        ...
                        <td className="py-3.5 px-3.5 text-right font-mono tabular-nums text-slate-400 hidden lg:table-cell">
                          {formatCompact(coin.total_volume)}
                        </td>
                        <td className="py-3.5 px-3.5 text-right font-mono tabular-nums font-bold text-slate-300">
                          {formatCompact(coin.market_cap)}
                        </td>
                        <td className="py-3.5 px-3.5 text-right">
                          <button
                            onClick={() => onOpenCryptoConverter(coin)}
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white/[0.04] hover:bg-blue-600 hover:text-white text-slate-300 text-xs font-semibold transition-all border border-white/[0.06] cursor-pointer active:scale-95"
