...
export default function ForexTerminal({ rates = DEFAULT_RATES, source, lastUpdated, onRefresh }) {
  ...
  return (
    <section id="forex-terminal" className="py-12 border-b border-white/[0.06]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[11px] font-mono text-emerald-400 bg-[#0C1017] border border-white/[0.08] px-3 py-1.5 rounded-xl font-semibold flex items-center gap-2 shadow-sm">
            ...
            return (
              <div
                ...
                className={`p-3.5 rounded-2xl border transition-all cursor-pointer group shadow-sm ${
                  isSelected
                    ? 'bg-blue-600/15 border-blue-500/60 shadow-blue-500/10'
                    : 'bg-[#0C1017] border-white/[0.08] hover:border-white/[0.18] hover:bg-[#111622]'
                }`}
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-lg">{c.flag}</span>
                  <span className="text-[11px] font-mono font-bold text-slate-300">{c.label}</span>
                </div>
                <div className="text-base font-black text-white font-mono tabular-nums tracking-tight">
        ...
        <div className="bg-[#0C1017] border border-white/[0.08] rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mt-6 items-center">
            ...
            {/* Amount Input */}
            <div className="lg:col-span-4 bg-[#07090E] border border-white/[0.08] rounded-2xl p-4 transition-all focus-within:border-blue-500/60 focus-within:ring-2 focus-within:ring-blue-500/20">
              <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1 font-mono">
                Simulation Amount
              </label>
              <div className="relative flex items-center">
                <span className="text-xl font-bold text-slate-500 mr-2 select-none font-mono">
                  {baseObj.symbol || '$'}
                </span>
                <input
                  type="number"
                  ...
                  placeholder="0.00"
                  className="w-full bg-transparent border-0 p-0 text-2xl sm:text-3xl font-extrabold text-white font-mono tabular-nums tracking-tight focus:outline-none focus:ring-0"
                />
              </div>
            </div>

            {/* Base Currency Dropdown */}
            <div className="lg:col-span-3 bg-[#07090E] border border-white/[0.08] rounded-2xl p-4 transition-all focus-within:border-blue-500/60 focus-within:ring-2 focus-within:ring-blue-500/20">
              <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1 font-mono">
                From Currency
              </label>
              <div className="relative">
                <select
                  ...
                  className="w-full bg-transparent border-0 p-0 pr-6 text-sm sm:text-base font-bold text-white appearance-none focus:outline-none focus:ring-0 cursor-pointer"
                >
                  {CURRENCIES.map((c) => (
            ...
            <div className="lg:col-span-3 bg-[#07090E] border border-white/[0.08] rounded-2xl p-4 transition-all focus-within:border-emerald-500/60 focus-within:ring-2 focus-within:ring-emerald-500/20">
              <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1 font-mono">
                To Currency
              </label>
              <div className="relative">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <span className="text-xs text-slate-400 font-semibold font-mono mr-1">Presets:</span>
              ...
              <button
                ...
                className={`text-xs px-3 py-1 rounded-full border font-mono tabular-nums transition-all cursor-pointer ${
                  numericAmount === preset
                    ...
                    : 'bg-[#07090E] border-white/[0.08] text-slate-400 hover:text-white hover:border-white/20'
                }`}
              >
          ...
          <div className="mt-6 p-6 rounded-2xl bg-[#07090E] border border-white/[0.08] flex flex-col sm:flex-row sm:items-baseline justify-between gap-4">
            <div>
              ...
              <div className="flex items-baseline gap-3 flex-wrap">
                <span className="text-3xl sm:text-4xl lg:text-5xl font-black text-white font-mono tabular-nums tracking-tight">
            ...
            <button
              onClick={handleCopy}
              className="flex items-center gap-2 self-start sm:self-center px-4 py-2.5 bg-white/[0.06] hover:bg-white/[0.1] text-white rounded-xl text-xs font-semibold border border-white/[0.08] active:scale-95 transition-all cursor-pointer"
            >
                  ...
                  <div
                    ...
                    className={`p-3.5 rounded-2xl border cursor-pointer transition-all ${
                      isCurrent
                        ...
                        : 'bg-[#07090E] border-white/[0.06] hover:border-white/[0.15] hover:bg-[#111622]'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-lg">{curr.flag}</span>
                      <span className="text-xs font-bold text-slate-300 font-mono">{code}</span>
                    </div>
                    <div className="text-sm font-bold text-white font-mono truncate tabular-nums">
                      {curr.symbol} {converted.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                    </div>
                    <div className="text-[10px] text-slate-400 font-mono mt-1 truncate tabular-nums">
