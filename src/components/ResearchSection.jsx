...
export default function ResearchSection() {
  ...
  return (
    <section id="research" className="py-12 border-b border-white/[0.06]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 text-purple-400 text-xs font-semibold mb-2.5 border border-purple-500/20 font-mono uppercase tracking-wider">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-3.5 py-1.5 bg-[#0C1017] border border-white/[0.08] rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 font-mono"
            />
          </div>
        </div>

        {/* Category Pills */}
        <div className="flex items-center gap-2 overflow-x-auto pb-3 mb-6 border-b border-white/[0.06]">
          {categories.map((cat) => (
            <button
              key={cat}
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all cursor-pointer ${
                selectedCategory === cat
                  ...
                  : 'bg-[#0C1017] text-slate-400 hover:text-white border border-white/[0.06] hover:border-white/[0.15]'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPosts.map((post) => {
            ...
            return (
              <div
                key={post.id}
                className="bg-[#0C1017] border border-white/[0.08] hover:border-white/[0.18] rounded-3xl overflow-hidden transition-all duration-200 flex flex-col justify-between group shadow-xl"
              >
                <div>
                  {/* Thumbnail Banner */}
                  <div className="w-full h-44 overflow-hidden relative bg-[#07090E] border-b border-white/[0.06]">
                    {post.image ? (
                      <img
                        src={post.image}
                        alt={post.title}
                    ...
                    <span className="absolute top-3 left-3 text-[10px] font-bold bg-[#07090E]/90 text-blue-400 px-2.5 py-0.5 rounded-full border border-white/[0.1] backdrop-blur-md font-mono">
                    ...
                    {tags.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {tags.slice(0, 3).map((tag, idx) => (
                          <span key={idx} className="text-[10px] font-mono text-slate-500 bg-[#07090E] px-2 py-0.5 rounded border border-white/[0.04]">
                            #{tag.trim()}
                          </span>
                        ))}
                      </div>
                ...
                <div className="px-5 pb-5 pt-3 border-t border-white/[0.05] mt-auto flex items-center justify-between text-xs font-mono">
                  <button
                  ...
                  <Link
                    to={`/blog/${post.id}`}
                    className="inline-flex items-center gap-1 text-blue-400 hover:text-blue-300 font-bold group-hover:translate-x-0.5 transition-transform"
                  >
        ...
        {filteredPosts.length === 0 && (
          <div className="text-center py-16 bg-[#0C1017] border border-white/[0.08] rounded-3xl">
            <BookOpen className="w-8 h-8 mx-auto text-slate-600 mb-2" />
            <p className="text-sm text-slate-400 font-mono">No research reports matched "{searchQuery}".</p>
          </div>
        )}
      ...
      {activeDrawerArticle && (
        ...
        >
          <div 
            className="w-full max-w-xl h-full bg-[#0C1017] border-l border-white/[0.08] shadow-2xl p-6 sm:p-8 overflow-y-auto animate-in slide-in-from-right duration-300 flex flex-col justify-between"
          ...
          >
            <div>
              ...
              <div className="flex items-center justify-between pb-4 border-b border-white/[0.06] mb-6">
                ...
                <button
                  onClick={() => setActiveDrawerArticle(null)}
                  className="p-1.5 rounded-full hover:bg-white/[0.06] text-slate-400 hover:text-white transition-colors cursor-pointer"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="flex items-center gap-3 text-xs text-slate-400 my-4 font-mono">
              ...
              {activeDrawerArticle.image && (
                <div className="w-full h-56 rounded-2xl overflow-hidden mb-6 border border-white/[0.08]">
                  <img src={activeDrawerArticle.image} alt={activeDrawerArticle.title} className="w-full h-full object-cover" />
                </div>
              )}

              {/* Summary */}
              <div className="p-4 rounded-xl bg-[#07090E] border-l-4 border-blue-500 text-slate-300 text-sm italic mb-6">
